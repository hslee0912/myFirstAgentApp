...
- **(TBD)**  [A] Phase 8/9 — **S8 (orchestrator integration + DB schema migration) sub-decision D38 잠금 + 코드 적용**.
...
- **(TBD)**  [A] Phase 8/9 — **S7 (test_agent.js sub-decision D37 잠금 + 자동 사항 7개) + `agents/test_agent.js` 작성**.
...
- **(TBD)**  [A] Phase 8/9 — **S6 라운드 1 (lib/api_test.js sub-decision D33~D36) 잠금 + `lib/api_test.js` 작성**.
...
- **(TBD)**  [A] Phase 8/9 — **S5 라운드 2 (deploy_agent.js 내부 구현 sub-decision D28~D32) 잠금 + `agents/deploy_agent.js` 작성**.
...
- **(TBD)**  [A] Phase 8/9 — **S5 라운드 1 (deploy_agent.js orchestrator integration sub-decision D25~D27) 잠금**.
...
- **(TBD)**  [A] Phase 8/9 — **S4 라운드 3+4 (FE Dockerfile sub-decision D22~D24 + `.dockerignore` 추가) 일괄 처리** (사용자 명시 동의로 BE 패턴 그대로 채택, 옵션 비교 풀어쓰기 생략).
...
- **(TBD)**  [A] Phase 8/9 — **S4 라운드 1 (BE Dockerfile sub-decision D19~D21) 잠금 + `lib/stack_templates/BE/Dockerfile` 작성 + `stack.config.json`의 `BE.protectedConfigFiles` 갱신**.
...
- **(TBD)**  [A] Phase 8/9 — **S3 라운드 2 (D16~D18 sub-decision) 잠금 + `lib/stack_templates/docker-compose.yml` 작성**.
...
- **(TBD)**  [A] Phase 8/9 — **S3 라운드 1 (docker-compose.yml 구조 결정 D11~D15) 잠금** (추천 묶음 전체 채택).
...
- **(TBD)**  [A] Phase 8/9 — **S2 (부수 결정 D6~D10) 잠금** (추천 묶음 전체 채택).
...
- **(TBD)**  [A] Phase 8/9 (Deploy + Post-deploy Test) 구현 진행 시작 — 결정 라벨링 D1~D10 정의 + S1 잠금. 이전 plan-eng-review 세션의 D1~D11 plan은 plan-mode in-memory였고 본문이 어디에도 저장되지 않아 소실. → 메모리(`project_myfirstagentapp_roadmap.md` [A] 섹션) 및 `docs/ROADMAP.md` 1순위 섹션에 보존된 *잠긴 결정 5 + 부수 항목 5*를 D1~D10으로 재매핑.
