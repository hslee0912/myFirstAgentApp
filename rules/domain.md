...
### 4-ter. valid_credentials password 시드 매핑 (D92)

`POST .../login` 의 `valid_credentials` scenario는 **시드 사용자의 시드 password를 정확히 사용**해야 한다. 시드 hash가 *고정 값에 매핑*되어 있어 다른 password로는 `bcrypt.compare` false → 401 → scenario FAIL.

| 항목 | 값 | 위치 |
|---|---|---|
| 시드 username | `demo_user` | 명세서 §4-3 (player_users 시드) |
| 시드 password_hash (60자) | `$2b$10$fmKqy9chmRPcdIq6nEbRoeP7bNX7CPQmZ7felu0VOleDBqeUO96Aq` | 명세서 §4-3 |
| 위 hash와 매핑된 **평문 password** | **`Pass1234`** | 본 카탈로그 §2 password PASS 예 *첫 번째* |

→ `valid_credentials` scenario는 **반드시** `request_body.password = "Pass1234"`. 다른 §2 PASS 예 (`secure99x`, `userpass1`) 사용 시 시드 hash와 mismatch.

❌ **나쁜 예** (1cycle FAIL 사례 — 2026-05-20):
```json
"auth_login": { "test_scenarios": [{
  "name": "valid_credentials",
  "request_body": { "username": "demo_user", "password": "DemoPass1" }
  //                                                     ↑ 시드 hash와 mismatch → 401
}]}
```
...
"auth_login": { "test_scenarios": [{
  "name": "valid_credentials",
  "request_body": { "username": "demo_user", "password": "Pass1234" }
  //                                                     ↑ 시드 hash와 매핑됨 → 200
}]}
```

이 룰은 *모든 게임 공통 invariant* (시드 hash가 PoC 전체에서 고정값). SpecSync(Phase 2.8)가 drift type `credential_seed_mismatch`로 정적 차단 — `valid_credentials` scenario password ≠ `Pass1234`이면 FAIL → BE/CodeChecker 재진입.
