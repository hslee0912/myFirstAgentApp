...
function buildRetryUserPrompt({ fe_spec, api_contract, existing_files, allowed_paths, fix_instructions }) {
  ...
  return [
    '## 모드: RETRY (부분 수정)',
    '',
    '## ⚠️⚠️ RETRY 우선 점검 — D78 패턴 (vitest 5분 timeout 최빈 원인)',
    '',
    'retry round에서 LLM이 fix instructions 따라 *patch가 새 D78 사고를 만드는* 일이 잦음.',
    '아래 패턴을 *이번 응답에 새로 도입하지 말 것* (`rules/fe.md §4-ter-α` 참조):',
    '',
    '- 컴포넌트 시그니처에 `= () => {}`, `= [...]`, `= {...}` 같은 *inline default* +',
    '  그 prop이 useEffect dep array에 등장하면 → 매 render마다 새 reference →',
    '  effect re-run → setState → 무한 루프 → vitest 5분 timeout → round FAIL.',
    '',
    '✅ 안전한 3가지 패턴 (반드시 하나 채택):',
    '  (1) `const NOOP = () => {};` 모듈 상수로 default 분리 → 항상 같은 reference',
    '  (2) `useRef` 로 최신값 보관 + dep array에서 그 prop 제외',
    '  (3) useEffect dep array `[]` (mount once) — game loop / event listener 적합',
    '',
    '🚫 절대 금지 (cycle #4 FAIL 원인 추정):',
    '  `function GamePage({ onNavigate = () => {} }) { useEffect(() => { setX(...) }, [onNavigate]); }`',
    '',
    '## fe_spec (참고)',
    '```json',
    JSON.stringify(fe_spec, null, 2),
