...
function buildInitialUserPrompt({ be_spec, api_contract, existing_files, db_state }) {
  ...
  return [
    ...
    '',
    '## 라우터 mount 패턴 (필수)',
    '**모든 비즈니스 router는 `app.use("/api/v1/...", ...)` 형태로 mount해야 한다.** `api_contract.base_url`이 `/api/v1`이므로 모든 endpoint mount path는 이를 prefix로 가진다. 외부 진입 Nginx :80은 `/api/v1/*`만 BE 컨테이너로 forward하므로, 다른 prefix(`/api`, `/v1`, `/api/v2`)를 쓰면 외부 요청이 도달하지 못한다. 예시:',
    '```js',
    'app.use("/api/v1/auth", authRoutes);  // contract endpoint /auth/signup → 실제 mount /api/v1/auth/signup',
