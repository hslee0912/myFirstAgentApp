...
// D55 (2026-05-15): BE/FE 모두 Sonnet 4.5 (max output cap 64K)로 통일하고
// 한 번에 받는 산출물 크기를 늘려 continuation 횟수를 줄임. 30000 = Sonnet 4.5
// 안전 영역. CodeChecker는 schema-only 응답이라 호출처에서 별도로 작은 값 명시.
 ...
 */
async function callJSON({ system, user, agent, model, max_tokens = DEFAULT_MAX_TOKENS, cache, _retry = 0, _client }) {
  const MAX_LLM_RETRIES = 2;
  const MAX_CONTINUATIONS = 3;
  const resolvedModel = model || resolveModel(agent);
  const apiClient = _client || client;
  ...
  const initialUserContent = cache === 'user'
    ? [{ type: 'text', text: lastUser, cache_control: { type: 'ephemeral' } }]
    : lastUser;

  // ---------------- Continuation loop ----------------
  // Pattern 2 (Continuation): when stop_reason='max_tokens' the response was
  // cut. Instead of failing immediately, append the partial assistant text
  // and ask the model to continue from where it stopped. Accumulate until
  // stop_reason!='max_tokens' or MAX_CONTINUATIONS is reached.
  //
  // Cache behavior: only the initial user turn carries cache_control; the
  // assistant + 'continue' turns don't. The system prompt's cache_control
  // (when set) keeps hitting across all turns.
  ...
  const LLM_CALL_TIMEOUT_MS = Number(process.env.LLM_CALL_TIMEOUT_MS || 5 * 60 * 1000);
  ...
  for (let cont = 0; cont <= MAX_CONTINUATIONS; cont++) {
    ...
    const contTag = cont > 0 ? ` (continuation ${cont}/${MAX_CONTINUATIONS})` : '';
    console.log(
      `[llm:${agent || '?'}] usage: input=${u.input_tokens || 0}t output=${u.output_tokens || 0}t, ` +
      `stop_reason=${response.stop_reason}${contTag}`
    );
    ...
    console.warn(
      `[llm:${agent || '?'}] truncated at output ${u.output_tokens || 0}t — continuing ` +
      `(${cont + 1}/${MAX_CONTINUATIONS})`
    );
    messages = [
      ...
      { role: 'assistant', content: accumulated },
      {
        role: 'user',
        content:
          '위 응답이 max_tokens 한계로 중간에 잘렸다. 정확히 그 위치에서 이어서만 출력하라. ' +
          '앞 내용 반복 금지, 추가 설명 금지, 코드블록 금지. ' +
          '최종 누적 결과가 단일 valid JSON object가 되도록 마무리하라.',
      },
    ];
  }
  ...
  const outputBudgetUsed = (lastUsage.output_tokens || 0) / max_tokens;
  if (lastStopReason === 'max_tokens') {
    // All continuations exhausted, still truncated.
    console.error(
      `[llm:${agent || '?'}] ❌ STILL TRUNCATED after ${MAX_CONTINUATIONS} continuations. ` +
      `last output_tokens=${lastUsage.output_tokens || 0}, total output=${totalOutput}t.`
    );
    if (_retry < MAX_LLM_RETRIES) {
      console.warn(
        `[llm:${agent || '?'}] retrying full call with stricter prompt ` +
        `(${_retry + 1}/${MAX_LLM_RETRIES + 1})...`
      );
      return callJSON({
        system, user, agent, model, max_tokens, cache, _retry: _retry + 1, _client,
      });
    }
    const truncErr = new Error(
      `LLM response TRUNCATED after ${MAX_CONTINUATIONS} continuations + ${MAX_LLM_RETRIES + 1} full attempts. ` +
      `total_output=${totalOutput}t (last call hit cap=${max_tokens}, model=${resolvedModel}). ` +
    ...
    console.warn(
      `[llm:${agent || '?'}] WARN last call output near cap: ` +
      `${lastUsage.output_tokens}/${max_tokens} (${(outputBudgetUsed * 100).toFixed(1)}%). ` +
  ...
  try {
    ...
    const diagnostic =
      ...
      `\n  ─ total output: ${totalOutput}t (across ${1 + Math.min(MAX_CONTINUATIONS, totalOutput > 0 ? totalOutput : 0)} call(s))` +
      `\n  ─ last call output_tokens: ${lastUsage.output_tokens || 0} / max_tokens: ${max_tokens}` +
      '\n  ─ stop_reason was end_turn — not a truncation issue. Likely a string-escape or JSON-syntax issue inside the response body.' +
