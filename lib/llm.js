 ...
 * @param {boolean} [opts.cache] - if true, mark system prompt with ephemeral cache_control.
 *                                  Saves ~90% on cached input tokens for repeated calls
 *                                  within the 5-min TTL (e.g. Phase 4 retries). System
 *                                  prompt must be ≥1024 tokens (Sonnet/Opus) or ≥2048
 *                                  tokens (Haiku) for caching to activate.
 * @returns {Promise<Object>}
 */
async function callJSON({ system, user, agent, model, max_tokens = DEFAULT_MAX_TOKENS, cache = false }) {
