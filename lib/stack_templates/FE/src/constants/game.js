...
//
// ⚠️⚠️ **적 발사도 *반드시* PATTERN_DEFINITIONS lookup 거칠 것** (D97-bis-3, 2026-05-21):
//    fireEnemyBullet / spawnEnemyBullet 같은 함수가 *직접 aimed 단발*만 만들면
//    모든 적이 동일 발사 패턴이 됨 (관측된 사고). 룰:
//      1) 적 발사 함수도 enemy.weaponId → WEAPONS.find → pattern → PATTERN_DEFINITIONS lookup.
//      2) *플레이어 발사 함수와 동일한 코드 경로* (helper 1개로 합치는 게 권장).
//      3) 기준 각도만 다름 — 플레이어=-π/2, 적=atan2(player-enemy).
//      4) 같은 bullets/speedMul/pierce/fixedDirection 4 필드를 *그대로* 적용.
//    🚫 적 발사를 단순 `BULLET_SPEED * 0.7 * unit(dx,dy)` 같은 hardcode로 처리하면 안 됨.
//
// ⚠️⚠️ **적 발사도 *반드시* PATTERN_DEFINITIONS lookup 거칠 것** (D97-bis-3, 2026-05-21):
//    fireEnemyBullet / spawnEnemyBullet 같은 함수가 *직접 aimed 단발*만 만들면
//    모든 적이 동일 발사 패턴이 됨 (관측된 사고). 룰:
