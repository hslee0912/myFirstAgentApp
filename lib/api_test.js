 ...
 *
 * Handles two CodeChecker output dialects (the canonical format and a legacy
 * "response.{success,error_cases}" format that some LLM responses still emit):
 *   - endpoint.path                   ← prefixed with contract.base_url (if set)
 *   - endpoint.responses              ← always an object keyed by status code
 *
 * Idempotent: re-normalizing an already-normalized contract is a no-op (the
 * base_url is consumed at the contract level; endpoint.path keeps the full
 * value after first normalization).
 *
 * Legacy mapping:
 *   response.success = { status_code, schema }   → responses[String(status_code)] = { schema }
 *   response.error_cases[i] = { status_code, ... } → responses[String(status_code)] = { schema? }
 *
 * @param {Object} contract
 * @returns {Object} a new contract object with normalized endpoints
 */
function normalizeContract(contract) {
  ...
  const normalized = endpoints.map((ep) => {
    ...
    // Only prefix base_url when ep.path doesn't already start with it (idempotent).
    const path = baseUrl && !rawPath.startsWith(baseUrl) ? `${baseUrl}${rawPath}` : rawPath;
...
async function runContract({ baseUrl, contractPath }) {
  const contract = normalizeContract(loadContract(contractPath));
  const endpoints = Array.isArray(contract.endpoints) ? contract.endpoints : [];
