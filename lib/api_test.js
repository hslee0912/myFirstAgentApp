 ...
 */
/**
 * D68 (2026-05-18): subset 매칭 — actual 응답 객체가 expected의 모든 key/value를
 * *포함*하는지 재귀 검증. expected의 nested 객체는 deep subset. expected에 없는
 * actual의 추가 필드는 허용.
 */
function checkSubset(actual, expected, dotPath) {
  const errors = [];
  if (expected === null) {
    if (actual !== null) errors.push(`${dotPath}: expected null, got ${JSON.stringify(actual)}`);
    return errors;
  }
  if (typeof expected !== 'object' || Array.isArray(expected)) {
    if (actual !== expected) {
      errors.push(`${dotPath}: expected ${JSON.stringify(expected)}, got ${JSON.stringify(actual)}`);
    }
    return errors;
  }
  if (typeof actual !== 'object' || actual === null) {
    errors.push(`${dotPath}: expected object, got ${actual === null ? 'null' : typeof actual}`);
    return errors;
  }
  for (const [k, v] of Object.entries(expected)) {
    errors.push(...checkSubset(actual[k], v, `${dotPath}.${k}`));
  }
  return errors;
}

/**
 * D68 (2026-05-18): scenario 1건 실행.
 *
 * @param {Object} endpoint - normalize된 endpoint
 * @param {Object} scenario - { name, request_body?, request_query?, expect_status?, expect_response_subset? }
 * @param {string} baseUrl
 */
async function runScenario(endpoint, scenario, baseUrl) {
  ...
  if (method === 'GET' || method === 'HEAD') {
    const query = scenario.request_query || {};
  ...
  } else if (scenario.request_body && typeof scenario.request_body === 'object') {
    bodyForRequest = scenario.request_body;
  }
  trace.url = url;
  trace.request_body = bodyForRequest;
  trace.query_params = queryParams;

  let res, responseBody;
  try {
    const init = { method, headers: { 'Content-Type': 'application/json' } };
    if (bodyForRequest !== null) init.body = JSON.stringify(bodyForRequest);
  ...
  if (scenario.expect_status != null && res.status !== scenario.expect_status) {
    errors.push(`expected status ${scenario.expect_status}, got ${res.status}`);
  }
  if (scenario.expect_response_subset && typeof scenario.expect_response_subset === 'object') {
    errors.push(...checkSubset(responseBody, scenario.expect_response_subset, 'response'));
  }
...
async function runEndpoint(endpoint, baseUrl) {
  // D68 (2026-05-18): endpoint에 test_scenarios가 정의되면 *각 scenario 모두 실행*
  // → BE 실제 동작 (validation/edge case) 까지 검증. 없으면 기존 동작 (example로
  // positive case 1번).
  if (Array.isArray(endpoint.test_scenarios) && endpoint.test_scenarios.length > 0) {
    const scenarioResults = [];
    let allPass = true;
    for (const sc of endpoint.test_scenarios) {
      const r = await runScenario(endpoint, sc, baseUrl);
      scenarioResults.push(r);
      if (!r.pass) allPass = false;
    }
    return {
      pass: allPass,
      errors: scenarioResults.flatMap((r) => r.errors.map((e) => `${r.trace.scenario}: ${e}`)),
      trace: { endpoint: endpoint.path, method: endpoint.method, scenarios: scenarioResults.map((r) => r.trace) },
  ...
  const method = (endpoint.method || 'GET').toUpperCase();
  const errors = [];
  const trace = { endpoint: endpoint.path, method, resolved_path: resolvedPath };

  // Build request body/query from example (D35=A + D51=A)
  const requestSchema = endpoint.request && endpoint.request.schema;
  const example = requestSchema ? exampleBodyFromSchema(requestSchema) : null;

  // D51 (2026-05-15): GET/HEAD 요청은 request.schema를 *query string*으로 변환.
  //   사용자 보고 사고 (cycle 2): GET /api/v1/result/best가 query string 없이
  //   fetch → BE가 player_id 누락으로 400. 옛 흐름은 GET의 example body를
  //   *생성하기는 했지만 fetch init.body에선 빠뜨림*. → URL은 빈 채로 호출됨.
  //   해결: GET/HEAD에서 example의 *scalar 필드*를 URLSearchParams로 변환.
  //         객체/배열 필드는 query string 부적합이라 skip.
  let bodyForRequest = null;
  let queryParams = null;
  if (example && typeof example === 'object' && !Array.isArray(example)) {
    if (method === 'GET' || method === 'HEAD') {
      ...
      for (const [k, v] of Object.entries(example)) {
        if (v == null) continue;
        if (typeof v === 'object') continue;  // 객체/배열은 query string 부적합 — skip
        params.set(k, String(v));
      }
      const qs = params.toString();
      if (qs) {
      ...
      bodyForRequest = example;
    }
  }
  trace.url = url;                          // 최종 URL (query 포함)
  trace.request_body = bodyForRequest;      // POST/PUT/PATCH일 때만 채워짐
  trace.query_params = queryParams;         // GET/HEAD일 때만 채워짐
  ...
  try {
    ...
    if (bodyForRequest !== null) {
      init.body = JSON.stringify(bodyForRequest);
    }
    res = await fetch(url, init);
    const text = await res.text();
    try {
...
/**
 * D51 (2026-05-15): runEndpoint 안의 URL+body 구성 로직만 추출 — 단위 테스트
 * 용. fetch는 외부 부수효과라 격리.
 *
 * @returns {{ url: string, init: Object, queryParams: Object|null, requestBody: Object|null }}
 */
function buildFetchRequest(endpoint, baseUrl) {
  const resolvedPath = substitutePathParams(endpoint.path, endpoint);
  let url = `${baseUrl}${resolvedPath}`;
  const method = (endpoint.method || 'GET').toUpperCase();
  ...
  return { url, init, queryParams, requestBody: bodyForRequest };
}
