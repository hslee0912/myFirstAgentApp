...
const RETRYABLE_PATTERNS = [
  ...
  // D66 (2026-05-18): FE contract drift — FE 코드가 contract에 없는 endpoint fetch.
  // fe_agent.js의 inline retry 1회로 회복 못 한 경우 round retry로 escalate.
  {
    pattern: /Contract drift detected/i,
    category: 'FE_CONTRACT_DRIFT',
    hint:
      'FE 코드가 contract에 *없는* endpoint를 fetch. BE는 contract만 보고 mount하므로 그 fetch는 404 응답. ' +
      '해결 우선순위: (1) 해당 fetch 호출 제거 후 동등 UX를 contract endpoint만으로 달성 ' +
      '(예: 중복확인은 signup 시도 후 409 응답 처리), (2) contract에 *있는* endpoint로 교체. ' +
      'Agent는 contract를 임의 확장하지 말 것. 새 endpoint 필요 시 notes에 사유만 기록.',
  },
  {
