...
function checkSpecSync({ catalogPath, routerDir }) {
  ...
  // valid_credentials password seed 매핑 검사 (D92, 2026-05-20):
  //   POST .../login의 valid_credentials scenario는 시드 사용자의 시드 password를
  //   정확히 사용해야 함. 시드 hash가 PoC 전체에서 고정 값(`Pass1234`)에 매핑돼
  //   있어 다른 password 사용 시 bcrypt.compare false → 401 → scenario FAIL.
  //   카탈로그 §2 password PASS 예 *첫 번째*가 시드 매핑값으로 약속됨.
  const passwordSeedValue = catalog.fields.password
    && catalog.fields.password.passExamples
    && catalog.fields.password.passExamples[0];
  if (passwordSeedValue) {
    for (const [rf, detail] of Object.entries(allDetails)) {
      if ((detail.method || '').toUpperCase() !== 'POST') continue;
      if (!/\blogin\b/i.test(detail.path || '')) continue;
      const scenarios = Array.isArray(detail.test_scenarios) ? detail.test_scenarios : [];
      for (const sc of scenarios) {
        if (!sc.name || !/^valid_credentials/.test(sc.name)) continue;
        const pw = sc.request_body && sc.request_body.password;
        if (typeof pw !== 'string') continue;
        if (pw !== passwordSeedValue) {
          drifts.push({
            router: rf,
            scenario: sc.name,
            field: 'password',
            issue: 'credential_seed_mismatch',
            expected: `${JSON.stringify(passwordSeedValue)} (rules/domain.md §2 password PASS 예 첫 번째 = 시드 hash 매핑값)`,
            actual: pw,
          });
        }
      }
...
function buildFixInstructions(drifts) {
  ...
  for (const d of drifts) {
    ...
    } else if (d.issue === 'credential_seed_mismatch') {
      lines.push(`- ${d.router}: test_scenarios[name="${d.scenario}"].password = ${JSON.stringify(d.actual)} 는 시드 hash 매핑값과 다름. 시드 hash가 PoC 전체에서 고정값에 대응 → 다른 password 사용 시 bcrypt.compare false → 401. 정답: ${d.expected}`);
    }
  }
  lines.push('');
