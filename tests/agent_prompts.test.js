...
test('D43: prompt 길이 감소 확인 — 인라인 ~700 chars 제거되어 24,045보다 작음', () => {
  const be = require('../agents/be_agent');
  const sp = be._internal.SYSTEM_PROMPT;
  // D42 baseline 24,045 → D43 슬림화 23,347 → D44에서 rules/db.md에 D44 안내
  // 추가 + readSchemaSection 주석 보강으로 23,556. 다만 *D42 대비 감소*는
  // 여전히 유지 (24,045 > 23,556). 상한 24,000으로 완화 — D43 효과 유지 보장.
  assert.ok(sp.length < 24000, `SYSTEM_PROMPT total = ${sp.length} chars — D43 슬림화로 24,000 미만이어야 (D42 24,045 → 감소)`);
});
