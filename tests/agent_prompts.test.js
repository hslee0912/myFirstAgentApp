...
test('D43: prompt 길이 — 슬림화 효과 지속 + 후속 rules 보강 반영', () => {
  ...
  //   D45 25,117 (rules/be.md §3-zero 컨테이너 sanity 4 antipattern +~1,500)
  // 핵심: D43 슬림화 자체는 *여전히 유효* (만약 readSchemaSection의 D33 인라인이
  // 다시 들어오면 ~26,000+로 확 늘어남). 상한 26,000으로 완화해 후속 rules 추가
  // 여유 + 회귀(인라인 부활) 즉시 감지 보장.
  assert.ok(sp.length < 26000, `SYSTEM_PROMPT total = ${sp.length} chars — 26,000 미만 유지 필요 (D43 인라인 부활 감지)`);
});
